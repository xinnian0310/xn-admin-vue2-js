...
export default {
  name: 'XnIconPicker',
  components: { ArrowDown, CircleClose, xnAppIcon },
  props: {
  ...
  computed: {
    searchPlaceholder() {
      if (this.activeTab === 'element') return '搜索 Element 图标名，如 Setting'
      if (this.activeTab === 'iconify') return '筛选预设，如 home / dashboard'
      return '搜索本地 SVG 文件名'
    },
    displayList() {
      ...
      return ICONIFY_PRESETS.filter((name) => !kw || name.toLowerCase().includes(kw)).map(
        (name) => ({
          value: name,
  ...
  watch: {
    modelValue: {
      handler(val) {
        ...
        if (parsed) {
          this.activeTab = parsed.type
          if (parsed.type === 'iconify') this.iconifyInput = parsed.name
        }
      },
      immediate: true,
    },
  ...
  methods: {
    ...
    applyIconifyInput() {
      const name = this.iconifyInput.trim()
      if (!name.includes(':')) return
      this.activeTab = 'iconify'
      this.select(buildIconValue('iconify', name))
    },
