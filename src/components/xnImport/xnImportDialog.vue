...
export default {
  name: 'XnImportDialog',
  components: { CircleCheckFilled, Loading, UploadFilled },
  props: {
  ...
  emits: ['success'],
  setup() {
    return { Download: markRaw(Download) }
  },
  data() {
    return {
      visible: false,
  ...
  methods: {
    ...
    async handleDownloadTemplate() {
      try {
        await downloadExcelTemplate(this.columns, this.templateName)
      } catch (e) {
    ...
    async onFileChange(uploadFile) {
      ...
      this.result = null
      try {
        const parsed = await parseExcelFile(raw, this.columns)
        if (!parsed.length) {
        ...
        if (parsed.length > this.maxRows) {
          ElMessage.error(`超过单次导入上限 ${this.maxRows} 行`)
          return
        }
        const err = validateImportRows(parsed, this.columns)
        if (err) {
    ...
    async handleSubmit() {
      ...
      const err = validateImportRows(this.rows, this.columns)
      if (err) {
        ElMessage.error(err)
        return
      }
      ...
      try {
        const payload = mapImportRows(this.rows, this.columns)
        const res = await this.importer(payload)
