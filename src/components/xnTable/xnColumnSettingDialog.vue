...
export default {
  ...
  emits: ['update:modelValue', 'save', 'reset', 'closed'],
  data() {
    return {
    ...
    modelValue(val) {
      ...
      if (val) {
        this.rows = this.columns.map((col, index) => {
          const locked = this.isLocked(col)
          return {
            ...col,
  ...
  methods: {
    ...
    onDragOver(index) {
      ...
      if (this.isLocked(this.rows[index]) || this.isLocked(this.rows[this.dragIndex])) return
    ...
    handleSave() {
      this.$emit(
        ...
        this.rows.map((row, index) => ({
          ...row,
          label: this.isLocked(row) ? '选择框' : row.label,
