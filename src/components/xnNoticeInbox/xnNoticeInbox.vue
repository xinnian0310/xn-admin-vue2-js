...
export default {
  ...
  setup() {
    ...
    return { noticeStore, formatDateTime, resolveAttachmentUrl, openKkFileViewPreview }
  },
  computed: {
    activeAttachments() {
      return resolveAttachments(this.noticeStore.activeNotice)
    },
    contentHtml() {
      return decorateRichHtml(this.noticeStore.activeNotice?.content)
    },
    visible: {
      get() {
        return this.noticeStore.drawerVisible
      },
      set(val) {
        if (!val) this.noticeStore.closeDrawer()
        else this.noticeStore.openDrawer()
      },
    },
    detailVisible: {
      get() {
        return !!this.noticeStore.activeNotice
      },
      set(val) {
        if (!val) this.noticeStore.closeDetail()
      },
    },
  },
}
