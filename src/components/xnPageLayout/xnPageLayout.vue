...
export default {
  ...
  emits: ['update:viewMode', 'update:page', 'update:pageSize', 'page-change'],
  computed: {
    hasAside() {
      return !!this.$slots.aside
    },
    hasTableOrCard() {
      return !!this.$slots.table || !!this.$slots.card
    },
    showViewSwitchActive() {
      return this.showViewSwitch && !!this.$slots.table && !!this.$slots.card
    },
    hasToolbar() {
      return !!this.$slots.toolbar || !!this.$slots['toolbar-extra'] || this.showViewSwitchActive
    },
    storageKey() {
      return `xn-view-mode:${this.$route.path}`
    },
    currentViewMode: {
      get() {
  ...
  watch: {
    '$route.path': {
      handler() {
        if (this.persistViewMode) {
          const stored = this.loadViewMode()
          if (stored !== this.viewMode) {
            this.$emit('update:viewMode', stored)
          }
        }
      },
      immediate: true,
  ...
  methods: {
    loadViewMode() {
      if (!this.persistViewMode) return this.viewMode
