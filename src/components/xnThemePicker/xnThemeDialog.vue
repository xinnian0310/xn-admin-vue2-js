...
export default {
  name: 'XnThemeDialog',
  components: { Check, Delete, Moon, PictureIcon: Picture, Sunny, Upload },
  setup() {
    const themeStore = useThemeStore()
    return { themeStore, customColorFields, MAIN_BG_MAX_BYTES }
  },
  data() {
    return {
      activeTab: 'preset',
  ...
  created() {
    this.activeTab = this.themeStore.source
  },
  watch: {
    'themeStore.dialogVisible'(open) {
      if (open) this.activeTab = this.themeStore.source
    },
  },
  methods: {
    onColor(key, value) {
      if (!value) return
      this.themeStore.setCustomParts({ [key]: value })
    },
    onBgChange(file) {
      const raw = file.raw
      if (!raw) return
      ...
      reader.onload = () => {
        try {
          this.themeStore.setMainBgImage(String(reader.result))
        } catch (e) {
    ...
    clearBg() {
      try {
        this.themeStore.setMainBgImage(null)
      } catch (e) {
