...
export default {
  ...
  methods: {
    ...
    syncApiFromPrefix() {
      if (this.apiTouched) return
      const prefix = this.form.modulePrefix.trim()
      if (prefix) this.form.apiBasePath = `/api/${prefix}`
    },
    fileLabel(path) {
    ...
    open(row) {
      ...
      this.form.modulePrefix = this.defaultPrefix(row.path)
      this.form.apiBasePath = `/api/${this.form.modulePrefix}`
    ...
    async handleSubmit() {
      ...
      if (!formRef || !this.route?.id) return
      await formRef.validate(async (valid) => {
        if (!valid) return
        this.submitting = true
        try {
          const res = await generate(this.route.id, {
            modulePrefix: this.form.modulePrefix.trim(),
            apiBasePath: this.form.apiBasePath.trim(),
            persistPermissions: this.form.persistPermissions,
            generatePageUi: this.form.generatePageUi,
          })
    ...
    currentContent() {
      ...
      if (this.activeTab === 'sql') return this.result.sql
      return this.result.files.find((f) => f.path === this.activeTab)?.content ?? ''
    },
    async copyCurrent() {
    ...
    downloadZip() {
      if (!this.result?.zipBase64) {
        ElMessage.warning('无可下载内容')
        return
      }
      const binary = atob(this.result.zipBase64)
      ...
      a.href = url
      a.download = `codegen-${this.result.modulePrefix}.zip`
      a.click()
