...
export default {
  ...
  methods: {
    applyConfig(data) {
      ...
      this.donationTip = data.donation?.tip || ''
      this.tipSnapshot = this.donationTip
      this.qrcodes = (data.donation?.qrcodes ?? []).map((q) => ({
        label: q.label || '',
        src: q.src || '',
      }))
    },
    buildPayload(nextContacts = this.contacts, nextTip = this.donationTip, nextQrs = this.qrcodes) {
      return {
    ...
    async persist(payload, successMsg) {
      this.loading = true
      try {
    ...
    async saveDonationTip() {
      if (this.donationTip.trim() === this.tipSnapshot.trim()) return
      await this.persist(this.buildPayload(), '捐赠说明已保存')
    },
    resolveRowIndex(row, listLen) {
      const idx = Number(row.__index)
      if (Number.isInteger(idx) && idx >= 0 && idx < listLen) return idx
      return -1
    },
    onContactToolbar(action) {
      if (this.contactSelected.length !== 1) {
        ElMessage.warning('请选择一项操作')
        return
      }
      const row = this.contactSelected[0]
      const index = this.contacts.findIndex(
        (c) => c.label === row.label && c.value === row.value && c.link === row.link,
      )
      const resolved = index >= 0 ? index : this.resolveRowIndex(row, this.contacts.length)
      if (resolved < 0) {
    ...
    onQrToolbar(action) {
      ...
      const row = this.qrSelected[0]
      const index = this.qrcodes.findIndex((q) => q.label === row.label && q.src === row.src)
      const resolved = index >= 0 ? index : this.resolveRowIndex(row, this.qrcodes.length)
      if (resolved < 0) {
        ElMessage.warning('未找到选中项')
        return
      }
      if (action === 'edit') this.$refs.qrSaveRef?.open('edit', this.qrcodes[resolved], resolved)
      if (action === 'view') this.$refs.qrSaveRef?.open('view', this.qrcodes[resolved], resolved)
    },
    onContactTableAction(payload) {
      const index = this.resolveRowIndex(payload.row, this.contacts.length)
      if (index < 0) return
      const row = this.contacts[index]
      if (payload.action === 'edit') this.$refs.contactSaveRef?.open('edit', row, index)
      if (payload.action === 'view') this.$refs.contactSaveRef?.open('view', row, index)
    },
    onQrTableAction(payload) {
      const index = this.resolveRowIndex(payload.row, this.qrcodes.length)
      if (index < 0) return
      const row = this.qrcodes[index]
      if (payload.action === 'edit') this.$refs.qrSaveRef?.open('edit', row, index)
      if (payload.action === 'view') this.$refs.qrSaveRef?.open('view', row, index)
    },
    async onContactSaved(payload) {
      if (payload.mode !== 'edit' || payload.index == null) return
      const next = [...this.contacts]
      next[payload.index] = payload.data
      await this.persist(this.buildPayload(next), '更新成功')
    },
    async onQrSaved(payload) {
      if (payload.mode !== 'edit' || payload.index == null) return
      const next = [...this.qrcodes]
      next[payload.index] = payload.data
      await this.persist(this.buildPayload(this.contacts, this.donationTip, next), '更新成功')
    },
  },
}
</script>
