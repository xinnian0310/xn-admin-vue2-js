...
export default {
  ...
  computed: {
    infraCards() {
      return [
        { key: 'redis', title: 'Redis', ...this.infra.redis },
      ...
      return [
        {
          key: 'cpu',
          title: 'CPU 使用率',
          value: this.data.cpu.sysUsage,
          foot: `进程 ${this.data.cpu.processUsage}% · ${this.data.cpu.cores} 核`,
        },
        {
          key: 'mem',
          title: '内存使用率',
          value: this.data.memory.usage,
          foot: `${this.formatBytes(this.data.memory.used)} / ${this.formatBytes(this.data.memory.total)}`,
        },
        {
          key: 'jvm',
          title: 'JVM 使用率',
          value: this.data.jvm.usage,
          foot: `${this.formatBytes(this.data.jvm.used)} / ${this.formatBytes(this.data.jvm.max)}`,
        },
      ]
    },
  },
  ...
  methods: {
    ...
    gaugeOption(value) {
      const color = this.usageColor(value)
      return {
        animation: false,
        series: [
          {
    ...
    async load() {
      ...
      try {
        ...
        this.data = serverRes.data
        this.infra = infraRes.data
      } finally {
        this.loading = false
      }
    },
    ...
    async onRestart(name, title) {
      ...
      try {
        ...
        while (Date.now() < deadline) {
          ...
          await this.loadInfraOnly()
          const card = this.infraCards.find((c) => c.key === name)
          if (card?.status === 'UP') {
